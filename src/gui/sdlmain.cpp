		...
		if (in)	for (std::string line; std::getline(in, line); ) {
			...
			if (realnic_pos != std::string::npos &&
				...
				if (ne2000_section->Get_bool("ne2000")) {
					LOG_MSG("Enabled pcap Ethernet device during migration");
					Prop_bool* enable_prop = static_cast<Prop_bool*>(pcap_section->Get_prop("enable"));
					Prop_string* backend_prop = static_cast<Prop_string*>(ne2000_section->Get_prop("backend"));
					assert(enable_prop != NULL);
					assert(backend_prop != NULL);
					enable_prop->SetValue("true");
