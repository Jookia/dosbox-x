...
PcapEthernetConnection::PcapEthernetConnection(void)
      : EthernetConnection()
{
}
...
bool PcapEthernetConnection::Initialize(void)
{
	...
	if(pcapinst==NULL) {
		LOG_MSG("WinPcap has to be installed for the Pcap backend to work.");
		return false;
	}
	FARPROC psp;
...
void PcapEthernetConnection::SendPacket(uint8_t* packet, int len)
{
	pcap_sendpacket(adhandle, packet, len);
}

void PcapEthernetConnection::GetPackets(std::function<void(uint8_t*, int)> callback)
{
	int res;
